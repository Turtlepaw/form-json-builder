...
function FormPreview({ forms, displayForm, setDisplayForm }) {
    ...
    return (
        <Box>
            ...
            <Box border={`1px solid ${colorMode === 'dark' ? '#e3e5e8' : '#292b2f'}`} borderRadius='3px' height='fit-content'>
                ...
                <Box>
                    {forms[displayForm].modal.components.map(actionRow => (
                        <Box key={Math.random()} m='16px'>
                            <Text textTransform='uppercase' fontFamily='Helvetica Neue' fontWeight='500' fontSize='12px' color={colorMode === 'dark' ? '#4f5660' : '#b9bbbe'}>
